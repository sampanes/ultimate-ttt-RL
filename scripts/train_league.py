...
def main():
    ...
    ap.add_argument("--batch_opponents", action=argparse.BooleanOptionalAction, default=True, help="Batched path only (--parallel>0): batch the OPPONENT forward passes too, not just the active agent's. NN opponents (clones, archives, nn_big8, lottery) are deterministic argmax at eval time, so grouping them by weight and running one batched forward per group is byte-identical in outcome to the per-slot loop -- it just removes an unbatched Python-driven forward per opponent move. DEFAULT ON (gated 2026-07-02: verify_opponent_batch_parity PASS 80/80 + 1.09x A/B / 1.19x bench on RTX 3080 -- RESULT_PERF_BENCH.md). Pass --no-batch_opponents to fall back to the per-slot loop.")
    ap.add_argument("--amp",             action=argparse.BooleanOptionalAction, default=False, help="Mixed precision: fp16 autocast around the hot forwards + a GradScaler on the backward. DEFAULT OFF -- gated 2026-07-03: 0.99x speedup on RTX 3080 (convergence fine, just no win: tiny net, GPU is not the bottleneck -- RESULT_PERF_BENCH.md). CHANGES numerics, so re-gate via home_batch --phase perf before trusting it on different hardware.")
    ...
    if args.compile:
        if active.enable_compile():
            print("[perf] torch.compile ON (forward_both) -- first chunk pays a warmup compile.")
        else:
            print("[perf] --compile requires Triton for CUDA (no Windows build) -- "
                  "keeping eager forward, run continues uncompiled.")
