...
def calculate_games(hours):
    games_per_hour = GAMES_PER_SEC * 3600
    return int(games_per_hour * hours)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--a", "--agent", type=str, default="lottery", help="agent id to train (e.g. 'lottery', 'new_cnn')")
    parser.add_argument("--startnew", action="store_true", help="do not load any checkpoint")
    parser.add_argument("--overwrite", action="store_true", help="load latest checkpoint and save to it too")

    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--hours", type=validate_float, help=f"number of hours to train, assuming abysmal {GAMES_PER_SEC}/sec")
    group.add_argument("--games", type=validate_int, help="number of games to train")

    parser.add_argument("--opponent", type=str, default="random", help="opponent agent id (e.g. 'random', 'neural')")
    args = parser.parse_args()

    agent = get_agent(args.a)
    cfg = agent.cfg
    ...
    games_to_play = args.games if args.games else calculate_games(args.hours)
    print(f"Training for {games_to_play:,} games vs {args.opponent}...\n\nStarting {current_time_str}")

    if args.opponent == "self":
        agent_wins, opponent_wins, draws, shortest, longest, elapsed = train_against_self(agent, games_to_play)
    else:
        agent_wins, opponent_wins, draws, shortest, longest, elapsed = train_against_agent(agent, args.opponent, games_to_play)
