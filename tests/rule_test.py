...
def test_check_mini_win():
    ...
    input_and_expected = [
        ...
        # In-progress mini (no line, not full) returns EMPTY, not None -- matches
        # the engine contract (mini_winner does `if winner != EMPTY`).
        ([X, O, X, X, O, EMPTY, O, X, EMPTY], EMPTY)
        ]
