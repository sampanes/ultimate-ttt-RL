...
>
> **This is no longer the authoritative execution order.** The milestone board
> below (M0-M5.5) completed; the program that replaced it -- the 1,000 ms
> agent, then continual training on a frozen engine -- is not on this board at
> all. `PENDING.md` is likewise history rather than a queue.
>
> The three product goals below are still the right goals, with one correction:
> the "oracle champion, practically unbeatable, hosted" line has been overtaken.
> The measured result is that the **172k pocket beats the 6.77M oracle at equal
> wall clock**, so size is not the axis that separates them -- search under a
> deadline is. See `CURRENT_STATE.md` section 6.
>
> Direction: `ROADMAP.md`. Facts: `CURRENT_STATE.md`.

This was the authoritative execution order through M5.5.
...
Prep numbers in `RESULT_M3_PREP.md`: pocket base estimates ~1.29 MB int8 (4x under the 5 MB gate), ~0.8 ms/move on CPU vs ~1.1 ms on CUDA. Build WASM CPU path first; WebGPU is optional.
