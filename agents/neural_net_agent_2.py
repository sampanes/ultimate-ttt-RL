...
class NeuralNetAgent2(Agent):
    def __init__(self, cfg: ModelConfig, model_path: str = ""):
        ...
        if model_path:
            ...
            print(f"[!]\t{self.name}: no model_path given -- starting from RANDOM weights "
    ...
    def learn(self):
        ...
        Replaces the old MSE-regress-logit-toward-reward objective, which collapses
        the policy to ~uniform in symmetric self-play. The flat encoding carries no
        legality channel, so the softmax is over all 81 outputs; illegal moves are
        never selected at inference (select_move masks them) and drift down as they
        are never rewarded.
        """
    ...
    def clear_history(self):
        ...
        self.last_moves = []
        # self.last_players = [] Do I really not use this?
        self.last_rewards = []
